...
interface AnsewerWithUser extends Answer {
  user: User;
}
...
const CommunityPostDetail: NextPage = () => {
  ...
  const { data, isLoading, error } = useQueryWithMutate<IDetailPostResponse>({
    url: `/api/posts/${router?.query?.id}`,

    options: {
      enabled: !!router?.query?.id,
    },
    mutateQueryFn: (prev) => ({ ...prev, post: { ...prev.post } }),
  });
  console.log(data);
  const onValid = (validForm: TComment) => {};
  const { handleSubmit, register } = useForm<TComment>();
  const { mutate: wonder } = useMutation(`/api/posts/${router?.query?.id}`);
  useEffect(() => {
    if (data && !data.ok) {
      ...
      <div>
        ...
        <div className="mb-3 flex cursor-pointer items-center space-x-3 border-b px-4 pb-3">
          ...
          <div>
            ...
            <Link
              href={`/profile/${data?.post?.user.id}`}
